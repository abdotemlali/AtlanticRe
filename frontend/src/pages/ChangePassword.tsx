...
export default function ChangePassword() {
    ...
    // NOTE: Le JWT est un cookie httpOnly, jamais dans localStorage.
    // La page est accessible grâce au cookie déjà présent dans le navigateur.
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true)
        try {
            // Le cookie httpOnly est automatiquement envoyé par le navigateur (withCredentials: true)
            // Pas besoin d'header Authorization manuel
            ...
            setTimeout(() => {
                toast('Reconnectez-vous avec votre nouveau mot de passe', { icon: '🔑' })
                // Effacer la session locale et rediriger vers login
                localStorage.removeItem('auth_user')
                navigate('/login')
            }, 1500)
        } catch (err: any) {
            const detail = err?.response?.data?.detail
            // Si le cookie a expiré entre temps → rediriger vers login
        ...
        >
            ...
            >
                ...
                <div className="mb-6 border-b border-[var(--color-gray-100)] pb-4 text-center">
                    <h1 className="text-xl font-bold" style={{ color: 'var(--color-navy)' }}>Définir votre mot de passe</h1>
                    <div className="flex justify-center items-center gap-2 mt-2">
                        <Shield size={14} color="hsl(83,52%,36%)" />
                        <span className="text-xs font-medium" style={{ color: 'var(--color-gray-500)' }}>Première connexion</span>
                    </div>
                </div>

                <div className="bg-[#FFF8E7] border-l-[3px] border-[#F4C842] py-2 px-3 rounded text-[0.75rem] mb-5" style={{ color: 'hsl(30,88%,40%)' }}>
                    ⚠️ Vous utilisez un mot de passe temporaire. Définissez votre mot de passe personnel pour continuer.
                </div>
