// MODIFIÉ — Refonte complète de la page Analyse Globale (/analyse)
// Remplace l'ancienne page avec 2 onglets "Par pays global" / "Par marché (pays+branche)"
// Modifications implémentées :
// 1 : Onglet unique "Par pays" (fusion des 2 anciens onglets)
// 2 : Filtre local (Année synchro global, Branche multi, FAC/TTY/TTE, Vie/Non-vie)
// 3 : 4 jeux de params distincts (params, paramsNoBranch, paramsNoCountry, paramsNoYear)
...
// ─── Palette couleurs dynamiques par ordre de sélection ────────────────────
...
export default function Analysis() {
  ...
  // L'année est initialisée synchronisée avec le filtre global
  ...
  // pays_cedante est lu depuis le filtre GLOBAL (FilterPanel global)
  const buildFS = (overrides: Partial<FilterState> = {}): FilterState => ({
    ...
    pays_cedante: filters.pays_cedante,
    ...overrides,
  })

  // AJOUTÉ — Modification 3 : 4 jeux de paramètres distincts
  // params : normaux pour KPIs et Top branches (Année ✅, Branche ✅, FAC/TTY/TTE ✅, Vie ✅)
  ...
  // paramsNoBranch : immunisé branche pour Pie Chart Mix par branche (Branche ❌ Immunisé)
  const paramsNoBranch = useMemo(() => {
    return filtersToParams(buildFS({ branche: [] }))
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lf, filters.pays_cedante])

  // paramsNoCountry : immunisé pays pour garder la vue globale de la carte
  ...
  // paramsNoYear : sans année pour Évolution historique (Année ❌ Non)
  ...
  // Section "Par pays" — KPIs + tableau pays
  // S'applique : Année ✅, Branche ✅, FAC/TTY/TTE ✅, Vie/Non-vie ✅
  useEffect(() => {
    setLoadingPays(true)
    Promise.all([
  ...
  // Évolution historique — paramsNoYear (Année ❌, aucun filtre local)
  useEffect(() => {
    setLoadingYear(true)
  ...
  // Mix par branche (paramsNoBranch — Branche ❌ Immunisé)
  // Top branches (params — tous filtres ✅)
  useEffect(() => {
  ...
  // Couleur barre selon sélection et ordre
  ...
  // Segments sélectionnés → couleur dynamique, non sélectionnés → atténués
  ...
  return (
    <div className="space-y-6 animate-fade-in p-2 pb-12">
      ...
      {/* ── AJOUTÉ — Modification 2 : Filtre local ──────────────────────── */}
      <div className="bg-white rounded-xl border border-[var(--color-gray-100)] shadow-sm px-4 py-3">
        <div className="flex items-center justify-between mb-3">
          ...
          {/* Bouton reset aligné à droite — ne touche pas le filtre global */}
          <button
            onClick={handleReset}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* Année — synchronisée avec global, modifiable localement sans affecter les autres sections */}
          <div>
          ...
          {/* Branche multi-select — palette couleurs dynamiques par ordre de sélection */}
          <div>
          ...
          {/* FAC / TTY / TTE — colonne INT_SPC_TYPE */}
          <div>
      ...
      {/* ── MODIFIÉ — Section "Par pays" (Modification 1 : onglet unique fusionné) */}
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            {/* Badge "Vue filtrée" quand branche sélectionnée */}
          ...
          {/* Badges par branche sélectionnée — couleur dynamique + bouton X */}
          {isBranchFilterActive && (
        ...
        {/* KPI cards — recalculés strictement sur la sélection */}
        {loadingPays ? (
        ...
        {/* Carte monde — paramsNoCountry pour vue globale + selectedCountries pour highlight */}
        <div className="bg-white rounded-xl shadow-sm border border-[var(--color-gray-100)] p-5">
          {/* Badges pays sélectionnés dans l'en-tête */}
          ...
          {/* MODIFIÉ — WorldMap avec props params immunisés + pays sélectionnés */}
          <WorldMap
        ...
        {/* MODIFIÉ — Tableau pays : toutes lignes visibles, sélectionnées surlignées */}
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  {countryData.map((d: any, i: number) => {
                    // Modification 6 : fond coloré pour les lignes sélectionnées
                    ...
                    return (
                      <tr
                        ...
                        style={{ background: isSelected ? 'hsla(83,52%,36%,0.06)' : 'transparent' }}
                      >
      ...
      {/* ── Section Évolution historique — paramsNoYear (Année ❌, filtre immunisé) */}
      <div className="bg-white rounded-xl shadow-sm border border-[var(--color-gray-100)] p-5">
        ...
        {loadingYear ? (
          ...
          <>
            ...
            {/* Note visuelle — immunisation année */}
                ...
                <RechartsPieChart>
                  ...
                  >
                    {branchMixData.map((entry: any, index: number) => (
                      // Modification 4 : fill conditionnel — sélectionné → couleur dynamique, sinon atténué
                      <Cell
                        key={`cell-${index}`}
      ...
      {/* ── Section Top branches — Modification 5 : Bar Chart hybride */}
      <div className="bg-white rounded-xl shadow-sm border border-[var(--color-gray-100)] p-5">
        <div className="flex items-center gap-2 mb-4">
              ...
              >
                  ...
                  content={({ active, payload }: any) => {
                    ...
                    return (
                      <div className="bg-white/95 p-3 rounded-lg shadow-xl border border-[var(--color-gray-100)] text-xs">
                        ...
                        {/* Badge "Hors Top 10" si branche sélectionnée hors classement naturel */}
                ...
                >
                  {topBranchChartData.map((entry: any, i: number) => (
                    // Modification 5 : couleur dynamique si sélectionné, gris neutre sinon
                    <Cell
                      key={i}
